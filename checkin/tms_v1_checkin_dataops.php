...
switch ($op)
{
	...
	case "CONFIRMCHECK":
		...
		$queryReturn=$class_mysql_default$class_mysql_default->my_query("$selectReturn");
		if(!$queryReturn){
		...
		$queryerror=$class_mysql_default$class_mysql_default->my_query("$selecterror");
		if(!$queryerror){
		...
		while($rows = mysqli_fetch_array($result)) {
			...
			$result1 = $class_mysql_default$class_mysql_default->my_query("$queryString");
			if(mysqli_num_rows($result1) == 1) {
				...
				$resultsellticket = $class_mysql_default$class_mysql_default->my_query("$querysellticket");
	...
	//	$BalancePrice=0;  //这里先赋值为0？
		...
		$querymode=$class_mysql_default$class_mysql_default->my_query("$selectmodel");
	...
	/*	$queryString = "SELECT sfa_RunPrice FROM tms_bd_ServiceFeeAdjust WHERE sfa_DepartureSiteID = '{$rows2['st_FromStationID']}' 
		...
		$class_mysql_default$class_mysql_default->my_query("BEGIN");
	...
	/*	$queryString = "INSERT `tms_chk_CheckTicketTemp` (`ctt_TicketID`, `ctt_NoOfRunsID`, `ctt_LineID`, `ctt_NoOfRunsdate`, 
		...
		if(!$result) {
			...
			$retData = array('retVal' => 'FAIL', 'retString' => '检票数据添加失败！'.->my_error(), 'sql' => $queryString);
			echo json_encode($retData);
			exit();
		}
		  	...
		  	if(!$result) {
				...
				if(!$result) {
					...
					if($thisnoofruns==1){
						...
						$querychecktemp=$class_mysql_default$class_mysql_default->my_query("$updatechecktemp");
						if(!$querychecktemp){
							$class_mysql_default$class_mysql_default->my_query("ROLLBACK");
	...
	case "CONFIRMCHECKALL":
		...
		if(!$result) {
			$class_mysql_default$class_mysql_default->my_query("ROLLBACK");
			$retData = array('retVal' => 'FAIL', 'retString' => '检票数据添加失败1！'.->my_error(), 'sql' => $queryString);
			echo json_encode($retData);
			exit();
		}
		//检查报班车辆是否与票版中车辆一致
	...
	/*	$queryString = "SELECT `ctt_TicketID`, `ctt_NoOfRunsID`, `ctt_LineID`, `ctt_NoOfRunsdate`, `ctt_FromStationID`, `ctt_FromStation`, 
		...
		$result_ctt = $class_mysql_default$class_mysql_default->my_query("$queryString");
		if(!$result_ctt) {
	...
	/*	$selectbus="SELECT bi_BusID,bi_BusNumber,bi_BusTypeID,bi_BusType FROM tms_bd_BusInfo WHERE bi_BusID='{$BusID}'";
		$querybus=$class_mysql_default$class_mysql_default->my_query("$selectbus");
		if(!$querybus){
		  	...
		  	if(!$result) {
				...
				if(!$result) {
					...
					if($thisnoofruns==1){
						...
						$querycheck=$class_mysql_default$class_mysql_default->my_query("$updatecheck");
						if(!$querycheck){
							$class_mysql_default$class_mysql_default->my_query("ROLLBACK");
...
/*	case "CONFIRMPRINT":
	...
	case "CONFIRMPRINT":
		...
		if($opp!='reprint' || $error=='1'){
			...
			$queryticketmodel=$class_mysql_default$class_mysql_default->my_query("$selectticketmodel");
			if(!$queryticketmodel) {
			...
			if($oldBalanceNo!=''){
				...
				$queryupdate=$class_mysql_default$class_mysql_default->my_query("$updateBalanceInHandTemp");
				if(!$queryupdate) {
					$class_mysql_default$class_mysql_default->my_query("ROLLBACK");
					$retData = array('retVal' => 'FAIL', 'retString' => '结算单数据更新失败！', 'sql' => $queryString);
					echo json_encode($retData);	
			  	...
			  	if(!$result) {
					...
					$result = $class_mysql_default$class_mysql_default->my_query("$queryString");
				  	if(!$result) {
						...
						$resultLuggageCons = $class_mysql_default$class_mysql_default->my_query("$updateLuggageCons");
						if(!$resultLuggageCons ){
							...
							if(!$result){
								...
								$results = $class_mysql_default$class_mysql_default->my_query("$querys");
								if(!$results) {
									$retData = array('retVal' => 'FAIL', 'retString' => '查询检票数据失败！', 'sql' => $queryString);
									echo json_encode($retData);
								}
...
//								else 
	...
	case "GETPASSENGERINFO":
		...
		while($rows = mysqli_fetch_array($result)) {
			...
			$result2 = $class_mysql_default$class_mysql_default->my_query("$queryString");
			$seatStr = "";
	...
	case "withdraw":
		...
		$query = $class_mysql_default$class_mysql_default->my_query("$update");
		if(!$query) {
	...
	case "reprinte":
		...
		$query = $class_mysql_default$class_mysql_default->my_query("$select");
		if(!$query) {
	...
	case "changebus";
		...
		$resultSafetyCheck = $class_mysql_default $class_mysql_default->my_query("$selectSafetyCheck");
		...
		$resultselet = $class_mysql_default $class_mysql_default->my_query("$strsqlselet");
	...
	//	$rowmode= mysqli_fetch_array($querymode);		
		...
		$resultck = $class_mysql_default$class_mysql_default->my_query("$queryString");
		if(!$resultck) {
			$class_mysql_default$class_mysql_default->my_query("ROLLBACK");
		...
		/*	$queryString = "SELECT sfa_RunPrice FROM tms_bd_ServiceFeeAdjust WHERE sfa_DepartureSiteID = '{$rows2['ct_FromStationID']}' 
			...
			$queryupdatecheckt=$class_mysql_default$class_mysql_default->my_query("$updatecheckt"); 
		//	$ss=$ss.$TicketID;
		...
		$queryrt=$class_mysql_default$class_mysql_default->my_query($updatert);
		if(!$queryrt){
			$class_mysql_default$class_mysql_default->my_query("ROLLBACK");
			$retData = array('retVal' => 'FAIL', 'retString' => '更新调度信息失败！'.->my_error(), 'sql' => $queryString);
			echo json_encode($retData);
			exit();
		}
		...
		$queryct=$class_mysql_default$class_mysql_default->my_query("$updatect");
		if(!$queryct){
			$class_mysql_default$class_mysql_default->my_query("ROLLBACK");
			$retData = array('retVal' => 'FAIL', 'retString' => '更新车辆检票信息失败！'.->my_error(), 'sql' => $queryString);
			echo json_encode($retData);
			exit();
		}   
		$class_mysql_default$class_mysql_default->my_query("COMMIT");
